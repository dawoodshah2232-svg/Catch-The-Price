...
import { Mail, LockKeyhole, Eye, EyeOff, ArrowRight, ShieldCheck, AlertCircle } from 'lucide-react';
...
function LoginForm() {
  ...
  async function handleOAuth(provider: 'google' | 'apple') {
    ...
    if (error) {
      setErrorMessage(`Could not connect with ${provider}: ${error.message}`);
    }
  }
    ...
    <div className="w-full max-w-md">
      <div className="text-center mb-8">
        ...
        <p className="mt-2 text-sm text-[#546b62]">
          Sign in to access your saved products, price alerts, and deals.
        </p>
      </div>
      ...
      <div className="rounded-3xl border border-[#d6e3dd] bg-white p-6 sm:p-8 shadow-[0_16px_40px_rgba(12,25,19,0.06)]">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-[#1f382e] mb-1.5" htmlFor="login-email">
              Email address
            </label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="block text-xs font-bold text-[#1f382e]" htmlFor="login-password">
                Password
              </label>
              <a
                href={`/${country}/forgot-password`}
                className="text-xs font-bold text-[#00A859] hover:text-[#008f4c] hover:underline"
              >
                Forgot password?
              </a>
            </div>
        ...
        <div className="mt-6 flex items-center gap-3">
          <div className="h-px flex-1 bg-[#e2ece7]" />
          <span className="text-[11px] font-bold text-[#7a8f86] uppercase tracking-wider">or</span>
          <div className="h-px flex-1 bg-[#e2ece7]" />
        </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <button
            ...
            className="flex h-11 items-center justify-center gap-2 rounded-2xl border border-[#d2e0da] bg-[#fbfdfc] px-3 text-xs font-bold text-[#1f382e] hover:bg-[#f2f7f4] hover:border-[#b8cfc5] transition-colors"
          >
          ...
          >
            <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
              <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M15.97 6.37c.63-.77 1.06-1.84.94-2.92-.92.04-2.02.62-2.67 1.38-.58.67-1.08 1.76-.94 2.81 1.02.08 2.05-.53 2.67-1.27z" />
            </svg>
            <span>Apple</span>
          </button>
        </div>
        ...
        <div className="mt-7 text-center">
          <p className="text-xs text-[#546b62]">
            ...
            <a
              href={`/${country}/signup?next=${encodeURIComponent(next)}`}
