...
export function Header() {
  ...
  return <>
    ...
    <div className="xl:hidden bg-black border-b border-[#1d3540] px-3.5 pt-[max(env(safe-area-inset-top),9px)] pb-2.5"><div className="h-[42px] flex items-center justify-between gap-1"><a href={`/${country}`} aria-label="CatchThePrice Home" className="h-[40px] w-[112px] shrink-0 sm:w-[150px]"><img src="/images/catch-the-price-logo.png" alt="CatchThePrice" className="h-full w-full object-contain" /></a><div className="flex min-w-0 items-center gap-1"><CountrySwitcher compact/><span className="inline-flex h-10 items-center rounded-xl border border-[#67d7a2] bg-[#ECFDF5] px-2 text-[11px] font-semibold text-[#065F46]">{countryInfo.currency}</span><a href={`/${country}/account`} aria-label="Account" className="flex h-10 w-9 shrink-0 items-center justify-center rounded-xl border border-[#2a4957] bg-white/[0.04] text-[#8bf3c3] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#65E8A8]"><User className="w-[17px] h-[17px]"/></a></div></div></div>
