...
export function MobileBottomNav() {
  ...
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 ui-header backdrop-blur-xl border-t px-2 pb-[env(safe-area-inset-bottom,8px)] pt-1 shadow-[0_-8px_26px_rgba(24,52,43,0.08)]">
        ...
        {items.map((item) => {
          ...
          return (
            <a
              key={item.id}
              href={item.href}
              className={`flex flex-col items-center justify-center py-1.5 min-h-[52px] touch-target rounded-xl transition-all relative ${item.isActive ? 'text-[#08784B] font-extrabold' : 'ui-secondary hover:text-[#0B8F58]'}`}
            >
              <div className="relative flex items-center justify-center">
                <Icon className={`w-5 h-5 transition-transform duration-150 ${item.isActive ? 'scale-105 text-[#08784B]' : ''}`} />
                {item.badge !== null && item.badge !== undefined && (
                  <span className="absolute -top-1 -right-2.5 px-1.5 py-0.5 rounded-full text-[9px] font-extrabold bg-[#0B8F58] text-white ring-2 ring-white">{item.badge}</span>
                )}
              </div>
