...
export function PromoBannerRow() {
  ...
  return <section className="max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8 pb-5">
    <div className="grid md:grid-cols-3 gap-3">
      {promos.map(({title,text,cta,href,Icon,image,tone}) => <a key={title} href={href} className={`${tone} group relative h-[112px] sm:h-[120px] overflow-hidden rounded-md border border-black/[0.035] px-4 sm:px-5 py-3.5 sm:py-4 shadow-[0_3px_9px_rgba(15,43,31,.045)] transition-all duration-200 hover:-translate-y-px hover:shadow-[0_8px_16px_rgba(15,43,31,.1)]`}>
        ...
        <div className="relative z-10 flex h-full max-w-[58%] flex-col items-start"><div className="text-[16px] sm:text-[17px] font-black leading-[1.1] tracking-[-.04em] text-[#101a1e]">{title}</div><div className="mt-1 text-[10px] sm:text-[11px] font-medium leading-relaxed text-[#56676b]">{text}</div><span className="mt-auto inline-flex h-7 items-center gap-1.5 rounded-[5px] bg-[#087f4e] px-3.5 text-[9px] font-extrabold uppercase tracking-wide text-white shadow-[0_3px_7px_rgba(4,81,48,.25)] transition-colors group-hover:bg-[#056b40] group-hover:shadow-[0_5px_10px_rgba(4,81,48,.3)]">{cta}<ArrowRight className="w-3 h-3"/></span></div>
