...
export function StorefrontExtensions({ products, trending, dropped }: { products: Product[]; trending: Product[]; dropped: Product[] }) {
  ...
  return <>
    ...
    <section className="max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8 py-7 sm:py-12">
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-2 sm:gap-3 lg:grid-cols-4">{lifestyle.map(item => <a key={item.title} href={`/${country}/deals/all`} className="group relative h-[142px] sm:h-[174px] overflow-hidden rounded-lg bg-[#12221e] shadow-[0_7px_18px_rgba(7,35,25,.14)]"><img src={item.image} alt="" className="absolute inset-0 h-full w-full object-cover brightness-[.72] transition-transform duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#061b13]/88 via-[#061b13]/16 to-transparent" /><div className="relative flex h-full flex-col justify-end p-3 sm:p-4 text-white"><h3 className="text-[13px] sm:text-[17px] font-black tracking-[-.03em]">{item.title}</h3><p className="mt-0.5 hidden text-[10px] text-white/75 sm:block">{item.text}</p><span className="mt-2 sm:mt-3 inline-flex w-fit items-center gap-1 rounded bg-[#0c9b59] px-2 py-1 text-[8px] sm:px-2.5 sm:py-1.5 sm:text-[9px] font-extrabold uppercase tracking-wide">{item.cta}<ArrowRight className="h-3 w-3" /></span></div></a>)}</div>
    </section>
