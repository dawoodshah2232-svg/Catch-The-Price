...
export function PopularBrandsSection({products}:PopularBrandsSectionProps){ const {country}=useCountry(); const brands=topBrands(products); if(!brands.length)return null; return <section className="py-5 bg-white border-y border-[#e9edeb]"><div className="max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8"><div className="flex items-center justify-between mb-3"><h2 className="text-[18px] sm:text-[20px] font-black tracking-[-.025em] text-[#17242a]">Popular brands</h2><a href={`/${country}/search`} className="text-[11px] font-extrabold text-[#08784b] inline-flex items-center gap-1">View all brands<ArrowRight className="w-3.5 h-3.5"/></a></div><div className="flex gap-3 overflow-x-auto scrollbar-none pb-1">{brands.map(([brand])=>{const logo=brandLogos[brand.toLowerCase()]; const isApple=brand.toLowerCase()==='apple'; return <a key={brand} href={`/${country}/search?q=${encodeURIComponent(brand)}`} aria-label={`Shop ${brand}`} className="group flex h-[76px] w-[132px] shrink-0 items-center justify-center rounded-xl border border-[#e1ebe5] bg-[#fbfcfc] px-3 shadow-[0_1px_2px_rgba(16,73,46,.03)] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#9bd7b8] hover:bg-white hover:shadow-[0_7px_16px_rgba(16,73,46,.1)]">{logo ? <img src={`https://cdn.simpleicons.org/${logo}`} alt={brand} className={`${isApple ? 'h-8 max-w-[76px]' : 'h-11 max-w-[108px]'} object-contain transition-transform duration-200 group-hover:scale-105`} /> : <span className={`text-[20px] font-black text-center transition-transform group-hover:scale-105 ${brandMarkClass(brand)}`}>{brand}</span>}</a>})}</div></div></section>; }
